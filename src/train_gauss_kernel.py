...
from src.fid_score import gauss_get_Z_mmd_pushed_loader_stats, calculate_frechet_distance
...
def train_gauss_kernel(nets_for_pot,
                nets_for_pot_opt,
                encoder,
                encoder_opt,
                data_samplers,
    ...
    for epoch in tqdm(range(config.NUM_EPOCHS)):
        ...
        for it in tqdm(range(config.INNER_ITERATIONS)):
            ...
            for k in range(config.K):
                ...
                mu_T_XZ, log_sigma_T_XZ = encoder[k](XZ).chunk(2,dim=1) #[B*4,512]
                ...
                if it == config.INNER_ITERATIONS - 1:
                    wandb.log({f"mu: sq norm of generated latent by T{k}(x)":
                               torch.norm(mu_T_XZ, dim=1).square().mean().item()}, step=epoch)
                if it == config.INNER_ITERATIONS - 1:
                    wandb.log({f"sigma: sq norm of generated latent by T{k}(x)":
                               torch.norm(torch.exp(log_sigma_T_XZ), dim=1).square().mean().item()}, step=epoch)
                #======================#
                #  encoder step ends   #
                #======================#
